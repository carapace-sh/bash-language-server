...
for f in src/*.ts; do
  if [ -f "$f" ] && [ "${f%.d.ts}" = "$f" ]; then
    bun build "$f" --target node --outfile "out/$(basename $f)" --no-bundle
  fi
done
...
for f in src/util/*.ts; do
  if [ -f "$f" ]; then
    bun build "$f" --target node --outfile "out/util/$(basename $f)" --no-bundle
  fi
done
...
for f in src/shellcheck/*.ts; do
  if [ -f "$f" ]; then
    bun build "$f" --target node --outfile "out/shellcheck/$(basename $f)" --no-bundle
  fi
done
...
for f in src/shfmt/*.ts; do
  if [ -f "$f" ]; then
    bun build "$f" --target node --outfile "out/shfmt/$(basename $f)" --no-bundle
  fi
done
